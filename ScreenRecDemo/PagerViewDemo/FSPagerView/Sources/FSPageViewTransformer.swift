...
open class FSPagerViewTransformer: NSObject {
    ...
    open func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .overlap,.linear:
            ...
            NSLog("minimumScale = \(minimumScale)")
            NSLog("minimumAlpha = \(minimumAlpha)")
...
//            transform3D = CATransform3DTranslate(transform3D, -position*itemSpacing, 0, -position*itemSpacing);
...
//                transform = transform.translatedBy(x: 0, y: -ty)
                ...
                let x = 2*radius*sin(theta/2)*cos(theta/2)
                let y = 2*radius*sin(theta/2)*sin(theta/2)
                
                let scale = max(1 - (1-self.minimumScale) * abs(position), self.minimumScale)
                
//                transform3D = CATransform3DTranslate(transform3D, -position*itemSpacing, 0, ty)
//                transform3D = CATransform3DRotate(transform3D, rotation, 0, 0, 1)
//                transform3D = CATransform3DTranslate(transform3D, -position*itemSpacing, 0, -ty)
                
                transform3D = CATransform3DTranslate(transform3D, -x, 0, ty)
                transform3D = CATransform3DScale(transform3D, 0, 0, scale)
                transform3D = CATransform3DTranslate(transform3D, -x, 0, -ty)
